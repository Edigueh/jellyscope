...
export function App(): JSX.Element {
  ...
  useEffect(() => {
    ...
    // Re-fit the plot aspect box when the viewport changes.
    const onResize = () => resizeViewer();
  ...
  return (
    <div class="flex h-full w-full flex-col overflow-hidden bg-bg">
      {/* Docked toolbar — its own top row, nothing overlaps it. */}
      <header class="shrink-0 border-b border-border bg-surface-1">
        <Toolbar />
      </header>

      {/* Canvas + rail row. */}
      <div class="flex min-h-0 flex-1">
        {/* Canvas cell: centers the aspect-sized plot box; bg letterboxes. */}
        <div class="relative flex min-w-0 flex-1 items-center justify-center overflow-hidden">
          <div ref={canvasRef} />
          ...
          {/* Loading bar (top edge of the canvas cell) while a figure fetches. */}
          {chrome.loading && (
